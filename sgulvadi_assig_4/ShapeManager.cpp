...
{
	// find lower bound and upper bound for all the shapes
	if (theShapes.size() > 0)
	{
		...
		{
			if (aShape.getLowerBound().y < overalllowerB.y) overalllowerB.y = aShape.getLowerBound().y;
			if (aShape.getUpperBound().y > overallupperB.y) overallupperB.y = aShape.getUpperBound().y;
			if (aShape.getLowerBound().x < overalllowerB.x) overalllowerB.x = aShape.getLowerBound().x;
			if (aShape.getUpperBound().x > overallupperB.x) overallupperB.x = aShape.getUpperBound().x;
		}
		// call other function
...
void ShapeManager::paintEditIndicator()
{
	...
	YsGlDrawFontBitmap12x16("EDIT MODE");
...
bool ShapeManager::manage()
{
	...
	case FSKEY_MINUS: scale /= 1.05;
		break;
	case FSKEY_R: resetView(); // press R to reset view
		break;

	case FSKEY_E: inEditMode = !inEditMode; // press E to enter/exit edit mode
		break;
	}
	if (inEditMode)
	{// when in edit mode exit if no shapes are loaded
		if (theShapes.size() == 0)
		{
			cout << "No Shapes to edit!"<<endl<<"Press 'L' to load Shapes and press 'E' again to edit"<<endl;
			inEditMode = false;

		}
		else
		{
			paintEditIndicator();// let the user know they are in edit mode
			switch (key)
			{
			case FSKEY_COMMA:
				currShape -= 1;
				if (currShape < 0)// cycle through shapes
				{
			...
			case FSKEY_DOT:
				currShape += 1;
				if (currShape > theShapes.size() - 1)//cycle through shapes
				{
					currShape = 0;
			...
			case FSKEY_Z:// send the bounds of the chosen shape to reset view
				resetView(theShapes[currShape].getLowerBound(), theShapes[currShape].getUpperBound());
				break;

			case FSKEY_H: 
				if (currShape < theShapes.size() - 1)
					theShapes[currShape].changeHue();
			...
			case FSKEY_S:
				if (currShape == -1)
				{
					cout << "Choose a valid shape to save using '<' or '>' keys" << endl;
					break;
				}
				ofstream output_File;
				cout << "Enter the file name to save : ";
				cin >> fileName;
				if (fileName.find(".shape") == string::npos && fileName.length() > 0)
					fileName += ".shape"; 	
				output_File << theShapes[currShape];
				output_File.close();
				cout << "Saved '" << fileName << "' successfully.";
				break;
	...
	// all are call by value (output params - no input req)

	if (leftButton) {
		// write coords on screen if left button is held down - state 
		stringstream coordStream; // for displaying coordinates on screen
		Point2D screenPnt = { screenX * 1.f, screenY * 1.f };
		Point2D worldPnt = getWorldCoords(screenPnt);

		coordStream.str(""); //reset stream
		coordStream.precision(4); // stream : just like cin and cout
	...
	for (int i = 0; i < theShapes.size(); i++)
	{
		if (inEditMode == true)
		{
			if (currShape == i)//color the chosen shape differently
			{
				theShapes[i].paint(true, false, showThePoints, 200.f); 
				
			}

			else
			{
				theShapes[i].paint(true, false, false, 100.f); 
			}
		}
		else
		{
			
			theShapes[i].paint(true, false, false);//default
		}
	}
...
void ShapeManager::resetView(Point2D lowerB, Point2D upperB)
{
	...
	panX = winWidth / 2. - (upperB.x + lowerB.x) / 2. * scale; //instead of scale, could have used another function
