...
bool Shape2D::isContained(Point2D testPnt,float winWidthCoord, bool isInclusive)
{
	// create a point outside the window boundaries to the right, if we create a point 
	// using a random large number it can lead to round off errors, it is better to take
	// in the window width as an arguement and use that to creat the point at infinity
	Point2D pointInf;
	pointInf.x = winWidthCoord+1;// one larger than window boundary
	pointInf.y = testPnt.y;
	int intCount = 0;
	if (thePoints.size() >= 3) // if it is actually a polygon
	{
		Point2D prevPnt = thePoints.back();

		for (auto currPnt : thePoints) 
		{
			Point2D didIntersect = Line2D::getTrueIntersection(testPnt,pointInf,currPnt, prevPnt);
			if (didIntersect.x != -INFINITY || didIntersect.y != -INFINITY)// if there is actually a true intersection
			{	//if it is not intersecting vertices
				if ((didIntersect.x != currPnt.x && didIntersect.y != currPnt.y) || (didIntersect.x != prevPnt.x && didIntersect.y != prevPnt.y))
				{
					intCount += 1;
				}
				//if it is intersecting vertices check if it is lower vertex or upper
				//if it is lower increment count, if it is upper vertex continue
				else if (didIntersect.x == currPnt.x && didIntersect.y == currPnt.y)
				{
	
					if (prevPnt.y > currPnt.y)
					{
			...
			{
				if (isInclusive)//if poits on line are to be considered as inside the polygon
				{
					if (Line2D::isBetween(currPnt, prevPnt, testPnt))
					{
		...
		if (intCount % 2 != 0)// if count is odd the point is in the polygon
		{
	
			return true;
		}
		else// if even the point is not in the polygon
		{
			return false;
		}
	}
	else// if the geometry is not an enclosed polygon return false
	{
		return false;
	}
...
bool Shape2D::isContained(float testX, float testY, float winWidthCoord, bool isInclusive)
{
	Point2D testPnt;
	testPnt.x = testX;
	testPnt.y = testY;
	return isContained(testPnt, winWidthCoord, isInclusive);
}
