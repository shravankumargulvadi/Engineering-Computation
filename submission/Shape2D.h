...
protected:
	...
	float Ixx = 0.f;  // leave blank for now
	float Iyy = 0.f;  // leave blank for now
	float xBar = 0.f;  // leave blank for now
	float yBar = 0.f;  // leave blank for now
...
public:
	...
	bool isContained(Point2D testPnt, float winWidthCoord, bool isInclusive = true) ;
	bool isContained(float testX, float testY, float winWidthCoord, bool isInclusive = true);
